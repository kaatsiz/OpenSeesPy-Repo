...
# Trying to functionize :))

def singleColumnAnalysisFunction(nodalCoordinates, axialLoad):
    
    # # PREREQUISITES 

    #these need to be imported within scope of this function

    # import OpenSeesPy
    import openseespy.opensees as ops #apperently, can not define as * in a module. So assigned as ops.
    ...
    ops.node(11,nodalCoordinates[0],nodalCoordinates[1],nodalCoordinates[2])
    ops.node(12,nodalCoordinates[3],nodalCoordinates[4],nodalCoordinates[5])
    ...
    ops.geomTransf('Linear', colTransf, *colVector)
    ...
    # XBEAMS
    ops.element('elasticBeamColumn', 1010, *ele1010Nodes, A_beam, E_beam, G_beam, J_beam, Iy_beam, Iz_beam, colTransf)
    ...
    # ASSIGN POINT LOADS
    ops.load(12,*axialLoad) #for this method
    ...
    ops.test('EnergyIncr',tolerance,10)
    ...
    topNodeDisplacement=ops.nodeDisp(12) #3 displacements and 3 rotations for x y z directions. we will see only top node negative displacement.
    ...
    columnForce=ops.eleForce(1010) #THIS GIVES GLOBAL FORCE - 12 DOFS = 6 for bottom end 6 for top end. only Nodal force 100 is applied, therefore 100 is shown in vertical element forces at both ends.
    ...
    return supportReaction, topNodeDisplacement, columnForce
